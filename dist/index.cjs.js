    ...
    switch (action.type) {
        ...
        case 'CLEAR_CHANNEL_CARDS': {
            ...
            if (action.type === 'UPDATE_CHANNEL_STATE') {
                ...
                if (newState !== 'hidden') {
                    const currentActive = nextState.activeChannelId ? nextState.channels[nextState.activeChannelId] : null;
                    const isHigherPriority = !currentActive || updatedChannel.priority > currentActive.priority;
                    if (isHigherPriority) {
...
var css_248z = ".loading-indicator{align-items:center;display:flex;height:var(--overlay-collapsed-height,36px);justify-content:center;width:var(--overlay-collapsed-height,36px)}.loading-spinner{animation:loading-spin 1s linear infinite;border:3px solid rgba(0,0,0,.2);border-radius:50%;border-top:3px solid var(--overlay-primary-color);height:24px;width:24px}@keyframes loading-spin{0%{transform:rotate(0deg)}to{transform:rotate(1turn)}}";
styleInject(css_248z);

function LoadingIndicator() {
    return (jsxRuntimeExports.jsx("div", { className: "loading-indicator", role: "status", "aria-label": "Loading", children: jsxRuntimeExports.jsx("div", { className: "loading-spinner" }) }));
}
...
function OverlayCard({ channelId, card }) {
    ...
    return (jsxRuntimeExports.jsxs("div", { className: `overlay-card ${stateClass} ${directionClass}`, onClick: handleToggle, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, role: "button", tabIndex: 0, "aria-expanded": isExpanded, "aria-label": `Overlay Card - ${card?.title ?? ''}`, onKeyDown: (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                handleToggle();
            }
        }, children: [isLoading && (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx(LoadingIndicator, {}), card?.title && (jsxRuntimeExports.jsx("span", { className: "overlay-card-title", children: card.title }))] })), isIconOnly && card?.icon && (jsxRuntimeExports.jsx("div", { className: "overlay-card-icon", "aria-hidden": "true", children: card.icon })), !isLoading && !isIconOnly && card && (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [card.icon && (jsxRuntimeExports.jsx("div", { className: "overlay-card-icon", "aria-hidden": "true", children: card.icon })), jsxRuntimeExports.jsxs("div", { className: "overlay-card-content", children: [jsxRuntimeExports.jsx("h3", { className: "overlay-card-title", children: card.title }), jsxRuntimeExports.jsx("p", { className: "overlay-card-body", style: {
...
function OverlayPortal({ concurrencyMode = 'single', portalRoot, unstyled, sticky = false, position = 'top', }) {
    ...
    const activeChannels = Object.values(state.channels).filter((ch) => ch.cards.length > 0 || ch.state === 'loading' || ch.state === 'icon');
    ...
    const overlays = activeChannels
        ...
        return (jsxRuntimeExports.jsx(DefaultOverlay, { channelId: channel.channelId, cardId: activeCard?.id, sticky: sticky, position: position }, channel.channelId));
    });
...
function DefaultOverlay({ channelId, cardId, sticky, position, }) {
    ...
    if (!card && channel.state !== 'loading' && channel.state !== 'icon')
        return null;
    const portalClasses = [
        'overlay-portal',
